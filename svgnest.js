...
(function(root){
	...
	function SvgNest(){
		...
		this.launchWorkers = function(tree, binPolygon, config, progressCallback, displayCallback){
			...
			function placementWidthAndArea(placements, tree, binPolygon){
				...
				var usedArea = 0;
				var sheetArea = 0;
				var binHeight = binPolygon && binPolygon.height ? binPolygon.height : 0;
				...
				for(var i=0; i<placements.length; i++){
					...
					if(maxX !== null && binHeight > 0){
						sheetArea += Math.max(maxX, 0)*binHeight;
					}
				}
			...
			p.map(function(pair){
				...
				p2.map(worker.placePaths).then(function(placements){
					...
					if(!best || bestresult.fitness < best.fitness){
						...
						var utilizationStats = placementWidthAndArea(best.placements, tree, binPolygon);
